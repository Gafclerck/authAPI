...
from app.schema import VerifyCodeRequest
...
def send_code(user : Annotated[User, Depends(get_current_user)], session : Annotated[Session, Depends(get_session)]):
    if user.is_verified:
        raise HTTPException(
    ...
    code = generate_code()
    time_expires = timedelta(minutes=VERIFICATION_CODE_EXPIRE_MINUTES)
...
@router.post("/verify")
def verify(request: VerifyCodeRequest, session : Annotated[Session, Depends(get_session)]):
